...
- Cuando exista una duda pedagógica real, ejecutar spikes A/B aislados desde el
  mismo baseline y decidir por evidencia comparable.
...
Codex implementa e integra. Babylon Inspector y otras herramientas se cargan
sólo si la tarea las necesita; no se convierten en infraestructura global.
Gemini puede aportar análisis y MiniMax trabajo delegado por `mmx`, siempre bajo
...
- un jugador nuevo puede predecir una consecuencia física antes de ejecutarla;
...
El gate final normal es `npm run verify`; luego se juega el camino afectado.
