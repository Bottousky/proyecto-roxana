// Frontera única entre el shell y `three` para Physica. Sólo se alcanza por el
// `import()` dinámico de `loaders.ts`, así que `three` no entra al grafo estático
// del shell.
import type { ExperienceRuntimeModule, RuntimeHandle } from '../types.ts';
import { createPhysicaWorld } from './world.ts';
...
export const physicaRuntime: ExperienceRuntimeModule = {
  ...
  async mount(hostEl) {
    ...
    const handle: RuntimeHandle = {
      ...
      snapshot() {
        return { runtime: 'platformer-three', data: world.snapshot() as Record<string, unknown> };
      },
      pause() {
        world.pause();
      },
