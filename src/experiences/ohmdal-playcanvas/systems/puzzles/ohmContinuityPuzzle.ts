/**
 * Pure continuity puzzle for Ohm's first awakening (Arco I, B2).
 *
 * The player follows an observed interruption in the contact panel: one
 * bridge feeds Ohm and two bridges restore the long return path. The short
 * branch contains a visibly broken segment, so touching it cannot complete
 * the circuit. Rendering and input stay outside this model.
 */
...
export type ToggleResult =
  | {
      ...
      readonly action: 'placed' | 'removed';
      readonly state: PuzzleCircuitState;
      readonly message: string;
    }
  | {
      readonly success: false;
      readonly reason: 'broken' | 'no_supply' | 'unknown_gap';
      readonly message: string;
    };
...
export class OhmContinuityPuzzle {
  ...
  public canCover(gapId: string): boolean {
    const gap = OHM_PEDESTAL_GAPS.find((candidate) => candidate.id === gapId);
    if (!gap || gap.broken) return false;
    if (this.covered.has(gapId)) return true;
  ...
  public toggleGap(gapId: string): ToggleResult {
    ...
    const snapshot = this.getSnapshot();
    const stateMessage = snapshot.state === 'cerrado'
    ...
    return {
      ...
      action: 'placed',
      state: snapshot.state,
      message: `${stateMessage} Puentes disponibles: ${snapshot.supplyLeft}/${this.supplyTotal}.`,
    };
  }
  ...
  public getSnapshot(): OhmPuzzleSnapshot {
    ...
    const gaps: OhmGapView[] = OHM_PEDESTAL_GAPS.map((gap) => ({
      ...
      energized: this.covered.has(gap.id) && energizedNodes.has(gap.fromNode),
    }));
  ...
  private computeEnergizedNodes(): Set<string> {
    ...
    while (queue.length > 0) {
      ...
      for (const next of this.getConductingNeighbors(current)) {
  ...
  private getConductingNeighbors(node: string): string[] {
    ...
    // Solid segments in the observed path.
    ...
    if (node === 'OHM') {
      neighbors.push('CRUCE_ALTO', 'NUDO');
    }
    if (node === 'NUDO') neighbors.push('OHM', 'ABAJO_ESTE');
    if (node === 'ABAJO_ESTE') neighbors.push('NUDO');

    if (node === 'ABAJO_OESTE') neighbors.push('OESTE_ALTO');
    if (node === 'OESTE_ALTO') neighbors.push('ABAJO_OESTE', 'FUENTE_MENOS');
    if (node === 'FUENTE_MENOS') neighbors.push('OESTE_ALTO');

    for (const gap of OHM_PEDESTAL_GAPS) {
      if (gap.broken || !this.covered.has(gap.id)) continue;
  ...
  private isCircuitClosed(energizedNodes: Set<string>): boolean {
    if (!energizedNodes.has('OHM') || !energizedNodes.has('FUENTE_MENOS')) return false;
    return this.covered.has('g1') && this.covered.has('g5') && this.covered.has('g4');
  }
}
