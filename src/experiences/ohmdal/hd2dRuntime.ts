// Adaptador de `RuntimeHost` para el mundo HD-2D del slice.
//
...
//
// El mundo no reemplaza al runtime cenital de Ohmdal: convive con él. Quién de los
// dos sirve una ubicación lo decide `ExperienceLocation.runtime` (`CP-021`).
import { createOhmdalWorld, type OhmdalWorld } from '../../ohmdal/world.ts';
 ...
 * antes las publicaba el `main.ts` del mundo, que ya no tiene el handle.
 *
 * Sólo en desarrollo, igual que el `__game` de `topdownRuntime`. El mundo no es entrada
 * de build (`CP-020`), así que fuera de `vite dev` esto no existe.
 */
function attachHarnessProbes(lab: OhmdalWorld): () => void {
...
export const hd2dRuntime: ExperienceRuntimeModule = {
  runtime: 'hd2d-three',
  async mount(hostEl) {
    const lab = createOhmdalWorld(hostEl);
    const detachProbes = attachHarnessProbes(lab);

    const handle: RuntimeHandle = {
      // El mundo es un espacio continuo: el recorrido cruza portal, taller y manantial
