...
assert.match(runtimeSource, /await zones\.preload\(destination\);[\s\S]*?await zones\.activate\(destination\);[\s\S]*?commit\(\)/, 'transition commits only after its zone is loaded and active');
assert.match(runtimeSource, /enterLoadedZone\('workshop', \(\) => \{\s*zones\.deactivate\('plaza'\)/, 'Taller entry retires Plaza rendering after its zone is ready');
assert.match(runtimeSource, /enterLoadedZone\('plaza', \(\) => \{\s*teleportPlayer\('workshop-to-plaza'\);\s*zones\.deactivate\('workshop'\)/, 'Taller exit restores Plaza before retiring the interior');
