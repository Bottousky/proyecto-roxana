...
> **Proyecto Roxana security override:** Never print any prefix or portion of
> `MESHY_API_KEY`, never ask the user to paste it into chat, never read it from project files,
> and never persist it automatically. Detect only whether the environment variable is present.
> Project rules and `docs/3d/MESHY_POLICY.md` take precedence over generic setup examples.
...
if [ -n "${MESHY_API_KEY:-}" ]; then
  echo "MESHY_API_KEY_PRESENT=true"
else
  echo "MESHY_API_KEY_PRESENT=false"
fi
...
> 3. The key is only shown once — save it in a secure secret manager or user environment
>
...
Do not ask the user to provide the value in conversation. Pause and ask them to set
`MESHY_API_KEY` in their local user/session environment outside Codex, then restart the tool
session. Do not write `.env`, shell profiles, PowerShell profiles, or user environment variables
on their behalf. After restart, repeat the presence-only check above.
