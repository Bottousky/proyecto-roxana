...
try {
  ...
  await moveTo(180, -1.65, 0.2, 'Lighthouse calibration panel front');
  await pressTouchInteraction('calibrar Faro');
  await page.setViewportSize({ width: 844, height: 390 });
  await waitForSnapshot((state) => state.regionalInspection === 'lighthouse', 'panel Faro');
  assert((await page.getByTestId('lighthouse-trim-value').textContent()) === '0 V', 'El ajuste inicial debe coincidir con el modelo');
  await page.getByTestId('lighthouse-register-trim').click();
  await page.screenshot({ path: resolve(OUT, 'lighthouse-panel-operated.png') });
  await page.getByTestId('regional-maintenance-close').click();
  await page.setViewportSize(MOBILE_VIEWPORT);
  await moveTo(176.5, -1.65, 0.35, 'Lighthouse panel west aisle');
  await moveTo(176.5, 5.5, 0.25, 'Lighthouse beacon west aisle');
  await moveTo(180, 5.5, 0.15, 'Lighthouse beacon control from outside its foundation');
  ...
  await moveTo(176.5, 5.5, 0.3, 'Lighthouse west aisle back to feed');
  await moveTo(176.5, -3, 0.35, 'Lighthouse route around calibration plinth');
  ...
  await moveTo(176.5, -8, 0.35, 'Lighthouse return west aisle');
  await moveTo(176.5, 14, 0.35, 'Lighthouse return around beacon foundation');
